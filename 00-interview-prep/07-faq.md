...
|---|---|---|
| **RAG (Retrieval-Augmented Generation)** | A pattern that fetches relevant documents at query time and puts them in the LLM's prompt so the model answers from real data rather than guessing. | The most common production pattern for any LLM that needs domain-specific or up-to-date knowledge. |
| **Hallucination** | When an LLM generates confident but factually incorrect or unsupported statements. | The primary failure mode RAG and faithfulness evaluation are designed to detect and mitigate. |
| **Vector database** | A specialized database that stores embeddings and retrieves them by similarity. | The index that makes semantic retrieval fast at scale; common options are Pinecone, Qdrant, Weaviate, Milvus, and pgvector. |
| **Embedding** | A list of numbers (vector) that represents the semantic meaning of a piece of text. | Enables similarity-based search: texts with similar meanings end up with similar vectors. |
| **Fine-tuning** | Further training a pre-trained model on task-specific data to adjust its style, format, or domain behavior. | Bakes consistent behavior into the model's weights; paired with RAG for best results ("RAG for facts, fine-tuning for form"). |
| **Hybrid search** | Combining dense (embedding) and sparse (BM25 keyword) retrieval and merging the ranked results. | Catches both semantic synonyms and exact technical terms; now shipped natively by most vector databases. |
| **BM25** | A classic keyword-ranking algorithm that scores documents by term frequency weighted by document length. | The sparse retrieval arm of hybrid search; excels at exact product codes, function names, and rare nouns. |
| **Reciprocal Rank Fusion (RRF)** | A formula that merges multiple ranked lists into one by summing 1/(k + rank) scores across lists. | Combines dense and sparse retrieval results without requiring manually tuned weights. |
| **Reranking** | A second-pass step that re-scores the top retrieval candidates with a more accurate cross-encoder model. | Typically improves retrieval precision by 10-20 percentage points with about 100 ms of added latency. |
| **Contextual retrieval** | An Anthropic technique that prepends an LLM-generated context summary to each chunk before embedding, so chunks carry their place in the document with them. | Reduces retrieval failures by 49% (67% when combined with a reranker) by solving the context-loss problem of naive chunking. |
| **GraphRAG** | A RAG variant that builds a knowledge graph from the corpus and queries by entity and relationship traversal. | Handles aggregative questions ("summarize all risks across these 50 contracts") that vector RAG cannot because it returns disconnected chunks. |
| **Knowledge graph** | A data structure that stores entities and their relationships explicitly as nodes and edges. | Enables deterministic multi-hop reasoning over structured relationships that vector similarity search cannot capture. |
| **Chunking** | Splitting long documents into smaller pieces before embedding and indexing. | Balances retrieval precision (small chunks) against context richness (large chunks); structure-aware chunking is the highest-leverage improvement. |
| **Context window** | The maximum number of tokens an LLM can process in one call (input + output combined). | Sets the upper bound on what you can put in the prompt; long contexts cost more and attention degrades in the middle. |
| **Lost in the middle** | The finding that LLMs attend less to content in the middle of a long context than to content at the start and end. | Motivates RAG (filter the 0.01% of relevant data) even when using 1M-token context windows. |
| **Agent** | A system where an LLM decides which tool to call next, observes the result, and decides again in a loop until the task is complete. | Enables flexible multi-step task completion; requires different guardrails, sandboxing, and cost controls than single-turn LLM calls. |
| **ReAct** | An agent architecture that interleaves Reasoning (Thought) with Acting (tool call) and Observation in a loop. | The simplest and most widely used agent pattern; the mental model most interviewers expect candidates to know. |
| **MCP (Model Context Protocol)** | An open protocol standardizing how LLM applications connect to external tools and data sources, governed by the Linux Foundation's Agentic AI Foundation. | Universal in 2026 (Anthropic, OpenAI, Google, Microsoft, AWS all support it); there are over 2,300 public MCP servers. |
| **Agentic RAG** | A RAG pattern where the agent decides what to retrieve, evaluates whether the results are sufficient, and re-queries if not. | Replaces the fixed retrieve-then-generate pipeline with an adaptive loop that can do multi-hop retrieval. |
| **Context engineering** | Curating the full set of tokens an agent sees on every turn (system prompt, tools, retrieved data, history) to maximize relevance and minimize context rot. | Keeps long-running agents accurate; the core techniques are compaction, just-in-time loading, and sub-agent isolation. |
| **Context rot** | The gradual degradation in agent accuracy as the context window fills with stale tool outputs and intermediate results. | The primary reason long-running agents need active context management rather than just appending every result. |
| **Computer-use agent** | An agent that operates a desktop or browser by taking screenshots, deciding on actions, and executing mouse/keyboard inputs. | An emerging production pattern; Claude Sonnet 4.6 reaches 72.5% on OSWorld-Verified as of 2026. |
| **Agent Skills** | Modular instruction files (SKILL.md) that an agent loads on demand to specialize at a task. | Keep the system prompt small while giving the agent deep task-specific knowledge on demand. |
| **LLM-as-judge** | Using one LLM to score or evaluate the outputs of another LLM against a rubric. | Scales quality evaluation to millions of responses; must be calibrated against human judgments to detect and correct bias. |
| **RAGAS** | A Python evaluation framework providing automated metrics for RAG pipelines: faithfulness, answer relevance, context relevance, context recall. | The de facto standard eval toolkit; provides reference-free metrics computable on live production traffic. |
| **Golden test set** | A curated dataset of representative inputs with known correct outputs used to measure system quality. | The foundation of regression testing; must include hard cases and adversarial examples, not just easy ones. |
| **Model drift** | When a model's behavior changes over time due to silent provider updates or shifts in the traffic distribution. | Requires explicit version pinning, daily canary evals, and output distribution monitoring to detect early. |
| **KV cache** | A cache of Key and Value tensors computed during LLM generation so prior tokens do not need to be recomputed. | The mechanism behind efficient long-context inference; also the basis for prompt caching features at providers. |
| **Prompt caching** | Keeping the KV cache warm for a shared prompt prefix so subsequent requests pay 10-25% of the normal token cost. | Can cut per-request cost by 30%+ on workloads with a long static system prompt; cache writes cost 25% more. |
| **Speculative decoding** | A technique where a cheap draft model generates candidate tokens that the large target model verifies in one parallel pass. | Achieves 2-3x faster generation with zero quality loss; built into vLLM and TensorRT-LLM. |
| **vLLM** | An open-source LLM inference engine that pioneered PagedAttention and continuous batching. | The default open inference engine for self-hosted open-weight model serving. |
| **SGLang** | An open-source inference engine with roughly 29% higher throughput than vLLM on structured-output and function-calling workloads. | Preferred for structured generation at scale; has unpatched multimodal CVEs as of May 2026. |
| **TensorRT-LLM** | NVIDIA's inference engine that delivers 2-4x higher throughput than vLLM on NVIDIA GPUs. | The highest-performance option when you have committed NVIDIA capacity and can absorb 1-2 weeks of setup. |
| **Continuous batching** | A serving strategy that dynamically adds new requests to a running batch and removes completed ones. | Maximizes GPU utilization at all traffic levels; the key technique behind vLLM's throughput advantage. |
| **PagedAttention** | A KV cache management technique that allocates cache in virtual memory pages on demand. | Eliminates memory fragmentation and allows more concurrent requests per GPU. |
| **Semantic caching** | A cache that checks incoming queries against previously answered similar queries using embedding similarity. | Can eliminate the LLM call entirely for repeated or paraphrased questions, cutting cost and latency dramatically. |
| **Model cascading** | Routing simple queries to cheap small models and complex queries to expensive frontier models. | The highest-leverage cost optimization; typically achieves 60-80% cost reduction with minimal quality loss. |
| **Token budget** | A hard ceiling on token consumption per request, per user, or per agent task enforced at the API gateway. | Prevents runaway agent costs and ensures fair quota allocation across tenants. |
| **Episodic memory** | Stored summaries of past agent sessions, retrievable by similarity and recency. | Enables an agent to remember what happened in prior conversations across sessions. |
| **Semantic memory** | Extracted structured facts about the user or world stored persistently across sessions. | Enables personalization and cross-session continuity without replaying full conversation history. |
| **Mem0 / Zep / Letta / Cognee** | Production agent memory frameworks providing layered episodic, semantic, and procedural memory. | Build-vs-buy options for agent memory; each has different strengths for chatbot personalization, task agents, and KG-grounded RAG. |
| **Prompt injection** | An attack where malicious content in user input or a retrieved document overrides the model's system instructions. | OWASP LLM Top 10 #1 risk; cannot be fully escaped; requires defense-in-depth including isolation, guard models, and least-privilege tools. |
| **Indirect prompt injection** | Prompt injection where the malicious instruction is hidden in a document the model reads, not in the user's direct message. | More dangerous than direct injection because it can be triggered by any content the agent retrieves from the internet. |
| **OWASP LLM Top 10** | A canonical list of the top security risks for LLM applications (v2.0 released 2025; 2026 agentic update adds goal hijacking and cascading failures). | The industry reference for LLM security; knowing the top entries is expected at senior AI engineering level. |
| **Sandboxing** | Isolating code that an agent generates and runs from the host system using ephemeral containers or micro-VMs. | Prevents a prompt-injected agent from causing real damage; E2B, Docker, and Firecracker are the standard tools. |
| **Dual-LLM pattern** | Using a small guard model to classify user intent before the main model processes the input. | A defense against prompt injection and jailbreaks; the guard model sees the raw input before any retrieved content is added. |
| **Canary token** | A secret string embedded in the system prompt that, if it appears in the model's output, reveals that the system prompt was leaked. | Detects prompt extraction attacks and instruction-override attempts in production. |
| **DeepSeek V4** | A family of high-performance open-weight models from DeepSeek with extremely low API pricing (V4 Flash: $0.14/$0.28 per 1M). | Reset the cost floor for frontier-class models; a key data point for model routing and cost optimization decisions. |
| **Matryoshka embeddings** | Embedding vectors designed so any prefix truncation of the full vector retains most semantic quality. | Lets you choose the embedding dimension at query time to trade index size and speed against quality. |
| **MTEB (Massive Text Embedding Benchmark)** | A standardized benchmark suite covering retrieval, classification, clustering, and semantic similarity for embedding models. | The industry standard for comparing embedding models; always evaluate on domain-specific data in addition to MTEB. |
