...
- **Content moderation** (`DetectModerationLabels`) — flags explicit, suggestive, violent, or otherwise unsafe content using a hierarchical taxonomy; **Custom Moderation** lets you fine-tune with your own examples. ([Moderation](https://docs.aws.amazon.com/rekognition/latest/dg/moderation.html)) **Why:** User-generated content platforms cannot manually review millions of images. `DetectModerationLabels` automates the first-pass filter, and the hierarchical taxonomy (e.g., `Explicit Nudity > Graphic Nudity`) lets you set different confidence thresholds for different severity levels before routing borderline cases to human review.
...
- **Face comparison** (`CompareFaces`) — similarity between two faces in two images; no storage needed. **Why:** Comparing two specific images (e.g., selfie vs. ID photo) doesn't require a database — `CompareFaces` is a direct one-to-one similarity score without any storage overhead or recurring cost.
- **Collections** — you `IndexFaces` to store **face vectors** (mathematical templates, not the images) in a searchable container, then `SearchFacesByImage` / `SearchFaces` to find matches. This powers "is this person in our gallery?" use cases. ([Collections](https://docs.aws.amazon.com/rekognition/latest/dg/collections.html)) **Why:** Comparing one incoming face against thousands of stored faces with `CompareFaces` would require N sequential API calls. Collections index face vectors once and enable sub-second one-to-many search — the mathematical embedding captures the unique geometry of a face so searching is fast and the stored data is not a photo (privacy benefit).
- **Face Liveness** (`StartFaceLivenessSession`) — detects a real, present user vs. a spoof (photo/replay) during onboarding. **Why:** Biometric KYC can be defeated by holding up a printed photo or replaying a video. Face Liveness uses challenge-response motion prompts to confirm the subject is a live person, not a static image or recording.
...
**Video analysis**
- **Stored video** — async jobs (`StartLabelDetection`, `StartFaceSearch`, `StartContentModeration`, `StartSegmentDetection` for shots) that return results with timestamps; completion is signaled via Amazon SNS. ([Stored video](https://docs.aws.amazon.com/rekognition/latest/dg/video.html)) **Why:** Video is processed frame-by-frame across potentially thousands of frames; returning results synchronously would block for minutes. The async pattern (Start → SNS notification → Get) decouples submission from retrieval so your application stays responsive.
- **Streaming video events** — real-time processing off **Kinesis Video Streams** for connected-home labels (person/pet/package) and live face search. ([Streaming](https://docs.aws.amazon.com/rekognition/latest/dg/streaming-video.html)) **⚠️ Availability change: Streaming Video and Bulk Image Analysis are no longer available to new customers (effective April 30, 2026).** Existing accounts that used these features within the last 12 months retain access. New workloads should use the Rekognition Image API with frame extraction, or alternative streaming solutions. ([Source](https://docs.aws.amazon.com/rekognition/latest/dg/rekognition-availability-changes.html))
...
- Train a model to detect **your** objects/logos/defects using AutoML — as few as ~10 labeled images to start; you pay for training hours and per-hour inference while the model is running. Use when the built-in labels don't cover your niche (e.g., "our brand's logo," "cracked circuit board"). ([Custom Labels](https://docs.aws.amazon.com/rekognition/latest/customlabels-dg/what-is.html)) **Why:** The built-in `DetectLabels` vocabulary covers generic categories like "Dog" or "Car" but doesn't know your specific product SKUs, brand logos, or manufacturing defect patterns. Custom Labels trains a specialized model on your labeled images using AutoML, so you get domain-specific detection without writing model code — at the cost of needing to start/stop the inference endpoint explicitly to control billing.
...
- "Real-time / **live camera** face search or home security events" → **Rekognition streaming with Kinesis Video Streams** — but note this feature is **no longer available to new customers** (as of April 2026); the alternative is frame-extraction with Rekognition Image APIs.
