# Case Study: Eval-Gated CI/CD for an AI Product
...
Each golden set is built from three sources: production trace samples from the last 90 days (stratified by failure modes from error analysis), synthetic adversarial cases generated by a separate red-team LLM, and curated edge cases from customer support tickets.

> **Why (the rationale):** A golden set built only from synthetic cases will overfit to idealized input distributions and miss the messy real-traffic edge cases that cause actual customer escalations. Stratifying across three sources — production failures, adversarial synthetic, and support tickets — ensures the set covers the tail. Rotating 10–15% quarterly prevents the team from implicitly tuning prompts toward memorized test cases.
> **When to use:** Multi-source golden sets are necessary for customer-facing AI products where the failure distribution is heterogeneous. For narrow internal tools with a well-defined input domain, a curated synthetic set is sufficient and easier to maintain.
> **Nuances & gotchas:** The 1,200-case floor is calibrated for detecting a 2-point regression at 95% confidence given the judge's accuracy. If the judge's accuracy is lower (e.g., 75%), the required set size grows substantially. Archiving rather than deleting retired cases creates a growing historical set that nightly runs must stay bounded on, or the nightly cost scales unbounded over years. We rotate 10 to 15 percent of cases quarterly; we never delete cases (cases get archived to a frozen "historical regressions" set that runs only nightly). This avoids the over-fitting trap where the eval set drifts with the product.
...
The LLM judge is itself a model with prompt parameters and few-shot examples. We treat the judge prompt as a model and apply train/dev/test discipline:

> **Why (the rationale):** Without a dev/test split on the judge prompt, engineers iterate the few-shot examples against the same labeled cases used to validate the judge, silently overfitting the judge to the available labeled data. This means the judge appears accurate in internal testing but has inflated apparent accuracy, leading to miscalibrated block/pass decisions in CI.
> **When to use:** Apply train/dev/test discipline to the judge prompt whenever the judge is used for gating decisions (not just for monitoring). For pure monitoring where the judge score is informational, simpler calibration via a held-out sample is acceptable.
> **Nuances & gotchas:** The 30-day re-calibration cycle assumes the judge model itself is stable. When the underlying judge LLM is updated by the vendor (e.g., a mid-cycle Claude Sonnet update), the judge prompt may behave differently even without any changes to the prompt text — the 30-day cycle catches this, but a major model version bump should trigger an immediate out-of-cycle calibration. 60 percent of human-labeled cases tune the judge prompt, 20 percent select the best prompt variant, 20 percent are a hold-out we only consult before a major judge-prompt change. This pattern is the core of the [judgy methodology](https://github.com/ai-evaluation/judgy) and Hamel's eval posts.
...
Naive LLM-as-judge accuracy on subjective categories is around 75 to 88 percent in our domain.

> **Why (the rationale):** A raw judge score of 89% pass rate cannot be compared to a baseline of 87% when the judge itself has 15% error — the difference could be entirely judge noise. Statistical correction using the judge's confusion matrix converts the raw score into an unbiased estimate with a confidence interval. Gating on the CI lower bound means a PR is only blocked when the evidence of regression is statistically significant, not when the judge was unlucky on a few cases.
> **When to use:** Statistical correction is essential whenever the LLM judge's accuracy is below 95% and the gating tolerance is narrow (2–3 points). If the judge is very accurate (>95%) and the regression threshold is wide (>5 points), raw scores may be sufficient. Always use correction when reporting to auditors who expect rigorous methodology.
> **Nuances & gotchas:** The judgy correction depends on the judge's confusion matrix being stable across the PR's test distribution. If a PR introduces a new failure mode not well-represented in the held-out calibration set, the confusion matrix does not apply and the correction may be over- or under-estimating the true pass rate. New failure modes require re-calibration before the gate is trustworthy on that dimension. A raw judge score is biased. `judgy` computes a corrected estimate of the true pass rate using the judge's confusion matrix on the held-out set, and returns a confidence interval. We gate on the lower bound of the CI being within tolerance. This means we never block a PR on judge noise alone, and we never approve a regression that the judge merely failed to catch.
...
We do not score "quality" as a single number. We score along axes drawn from our failure-mode taxonomy:

> **Why (the rationale):** A composite quality score can mask critical regressions. A hallucination rate that increases by 3 points while format compliance improves by 5 points nets to a composite improvement that clears the gate — but the hallucination regression may be the specific failure mode that costs the next $4M renewal. Per-axis gating ensures each dimension has its own CI and block threshold.
> **When to use:** Per-axis gating is required when different failure modes have different business consequences (e.g., hallucination is catastrophic, minor format deviation is tolerable). A single composite score is acceptable only for products where all failure modes have comparable business impact.
> **Nuances & gotchas:** The taxonomy must be kept current with production failure patterns via quarterly error analysis. A frozen taxonomy from year 1 will miss newly emergent failure modes (e.g., agent-loop failures, multi-step reasoning errors) that were not present when the taxonomy was built, and they will silently pass through the gate ungated. hallucination, retrieval-miss, format violation, refusal, persona break, citation error. The taxonomy is the output of error analysis ([Hamel's open-coding + axial-coding pipeline](https://hamel.dev/blog/posts/field-guide/)) applied to 800 production failures over 6 months. Per-axis scores let us block on a hallucination regression even if overall quality improved.

### 5. Per-PR eval budget

A full eval-set run costs $80 to $200 depending on model spend. At 50 PRs per week, the naive cost is $4K to $10K per week.

> **Why (the rationale):** Running the full golden set on every PR is not economically viable at $80–200 per run × 50 PRs/week. Stratified sampling at 10–25% reduces per-PR cost to under $40 while preserving representation across all 12 failure modes. The nightly full-set run on `main` catches any regression that the per-PR partial sample missed.
> **When to use:** Stratified partial eval is the correct default for teams with more than ~20 PRs/week. For small teams (under 5 PRs/week) or teams with very high regression risk, running the full set on every PR is affordable and eliminates the sampling risk entirely.
> **Nuances & gotchas:** Stratified sampling guarantees representation across failure modes but not statistical power within each mode. A PR that regresses specifically on a rare failure mode with only 80 cases in the set (6.7% of 1,200) may produce a 10-case sample in a 12.5% run — too few cases to detect a 3-point regression with statistical confidence. Rare failure modes need a minimum-case-count floor applied to the stratification logic. We bound this:
