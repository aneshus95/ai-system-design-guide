...
- **Many pre-built connectors** — includes Amazon S3, Microsoft SharePoint (Cloud and Server), Microsoft OneDrive, Microsoft Teams, Microsoft Exchange, Salesforce, ServiceNow, Confluence (Cloud and Server), Google Drive, Gmail, GitHub (Cloud and Server), Jira, Slack, Box, Dropbox, Zendesk, Smartsheet, and more. Can also use **Amazon Kendra as its retriever**. **Why so many connectors:** Enterprise data lives in dozens of systems; Q Business needs to ingest from wherever the source-of-truth is, including respecting the ACLs of each source, so the connector catalog must grow to cover the actual enterprise software landscape. ([Supported connectors](https://docs.aws.amazon.com/amazonq/latest/qbusiness-ug/connectors-list.html))
- **Access-control-aware answers** — responses are limited to content the *asking user* is permitted to see. User identity comes from **AWS IAM Identity Center** (or IAM), and document ACLs from the source systems are honored. **Why IAM Identity Center is required:** Q Business needs to know *who* is asking so it can compare their identity against the ACLs ingested from each source system (e.g., SharePoint permissions). Without a federated identity provider, Q Business can't map the chat user to the same identity that governs document access in SharePoint or Salesforce — so answers would either be over-permissive (security risk) or impossible to personalize. ([Data & application security](https://docs.aws.amazon.com/amazonq/latest/qbusiness-ug/what-is.html))
...
- **User subscriptions** — per-user, per-month charge by subscription tier. Two tiers: **Q Business Lite** (approx. $3/user/month — basic Q&A, permission-aware responses up to one page) and **Q Business Pro** (approx. $20/user/month — all Lite features plus Q Apps, content creation, longer multi-page responses). **Why two tiers:** Not every employee needs the full generative capabilities; Lite lets organizations give read-only Q&A access cheaply while Pro unlocks richer generation for power users. (Verify current prices at exam time — AWS pricing can change.)
...
**Amazon Q Developer** — a **Free tier** (sign in with AWS Builder ID, no AWS account needed) plus a **Pro** tier (approx. $19/user/month) that raises limits and adds enterprise access controls, codebase customization, and organizational management. (Verify current price at exam time.) ([Q Developer pricing](https://aws.amazon.com/q/developer/pricing))
...
Because Q is built on Bedrock, **you don't pay Bedrock per-token separately** for Q — the model usage is bundled into Q's pricing. **Why bundled:** Q Business and Q Developer are finished applications with opinionated model choices; AWS prices them as SaaS subscriptions so you get predictable monthly costs rather than variable per-token bills that fluctuate with usage volume.
