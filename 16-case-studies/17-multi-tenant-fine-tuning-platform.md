# Case Study: Multi-Tenant Fine-Tuning Platform
...
A full 70B fine-tune per tenant costs about $4,500 in compute, produces a 140 GB artifact, and pins one H100.

> **Why (the rationale):** At 280 tenants doubling annually, full fine-tuning is economically and operationally impossible: 280 × 140 GB = 39 TB of artifact storage, 280 dedicated H100s for serving, and $4,500 per retrain per tenant. LoRA at r=16 collapses the artifact to 120 MB, shares the base model GPU, and costs $80–400 per retrain with only a 1.6-point accuracy gap — a gap acceptable given the 50x cost difference.
> **When to use:** LoRA is the right choice for multi-tenant scenarios where each tenant requires adaptation but the base capability is shared. Full fine-tuning is justified when a tenant's task distribution diverges radically from the base model's pretraining (e.g., highly domain-specific scientific notation) or when the 1–2 point accuracy gap is contractually unacceptable.
> **Nuances & gotchas:** The 120 MB artifact size assumes r=16 on attention layers (Q, K, V, O) only; extending to MLP layers roughly doubles the artifact size with diminishing returns. Base-model refresh forces re-training all tenant adapters simultaneously — 280 retrains every 6–9 months is the operational cost that is easy to underestimate when projecting staffing needs. LoRA at r=16 costs $80 to $400 per tenant per retrain, produces a 120 MB artifact, and shares the GPU. The accuracy gap on our internal contract-analysis eval is 1.6 points on a 100-point composite. We accept that gap because the cost differential is 50x and the operational story (hot-swap, ephemeral artifacts) is dramatically simpler. The Anyscale post linked above ran a similar comparison and reached the same conclusion.

### 2. Adapter swap budget and the noisy-neighbor problem

vLLM's multi-LoRA support keeps adapters in GPU memory but each adapter consumes a few hundred MB.

> **Why (the rationale):** With 280 tenants and ~30 GB available for adapters on an 80 GB H100 running int8, only ~200 adapters can be hot simultaneously. Without an explicit eviction policy, a bursty tenant evicts others' adapters, causing 200–600 ms cold-start latency spikes that violate the 1.2s p99 SLA. LRU with pinned slots for top-tier SLA tenants gives predictable latency to the customers who pay for it while letting low-traffic tenants tolerate occasional cold starts.
> **When to use:** Tiered eviction protection (pinned + LRU) is appropriate when tenants have differentiated SLAs and traffic patterns. If all tenants have equal SLAs and uniform traffic, simple LRU is sufficient.
> **Nuances & gotchas:** Traffic-aware pre-warming has a 78% hit rate; the remaining 22% are new or idle tenants who must pay the cold-start penalty. Pre-warming adds a per-node sidecar process that reads access logs — this sidecar must not become a performance bottleneck on nodes running at 95%+ GPU utilization during peak traffic. On an 80 GB H100 running a 70B base in int8 (about 40 GB), we have roughly 30 GB for adapters and KV cache. That budgets to roughly 200 adapters resident at once. We use LRU with traffic-aware pre-warming and tail-tenant pinning: the 30 tenants with strict latency SLAs are pinned and never evicted; the rest rotate. A tenant whose adapter is cold pays a 200 to 600 ms tail penalty. We surface that explicitly in the tenant SLA as a cold-start budget.
...
We treat the tenant's golden set as the product requirements document.

> **Why (the rationale):** A global quality metric averaged across all tenants would mask regressions on individual tenants — a new adapter that improves 279 tenants but degrades one by 8 points would pass a platform-level gate. Each tenant's legal team has encoded their specific drafting preferences and contract templates into the golden set; only a per-tenant gate catches regressions that matter to that tenant.
> **When to use:** Per-tenant eval gates are required in vertical-AI multi-tenant platforms where customers have invested in defining their own quality standards. A single shared eval is sufficient for horizontal platforms (e.g., a general coding assistant) where all users share the same quality dimensions.
> **Nuances & gotchas:** The 60–90 minute onboarding eval workshop is a bottleneck that scales linearly with tenant count. At 280 tenants doubling annually, this becomes a hiring constraint (CSM capacity) as much as an engineering one. Golden sets can also drift if tenants' legal preferences change post-onboarding; quarterly refresh is needed but easy to deprioritize in a growing business. The training pipeline runs the new adapter against that set after every retrain; if metrics regress more than 2 points on the composite, the artifact is held and a Slack ping goes to the tenant's CSM. This is the "eval-as-PRD" pattern Hamel Husain has written about ([How to construct domain-specific evals](https://hamel.dev/blog/posts/evals/)) and we extend it to be per-tenant. Each tenant's golden set is curated jointly with their legal team during onboarding (a 60- to 90-minute workshop) and refreshed quarterly.

### 4. Training-time isolation via Kubernetes namespaces plus network policy

Multi-tenancy is a defense-in-depth problem. Training jobs run in per-tenant namespaces;

> **Why (the rationale):** A training pipeline bug that reads from the wrong S3 bucket could bake Tenant B's confidential contracts into Tenant A's adapter — a legally catastrophic cross-contamination. Kubernetes namespaces + network policies + IAM role scoping create three independent failure-prevention layers; all three must fail simultaneously for data to cross tenant boundaries.
> **When to use:** Defense-in-depth isolation (namespace + network policy + dedicated IAM) is required whenever training data is confidential and cross-tenant contamination has legal or contractual consequences. Shared-namespace training with only IAM isolation is sufficient for low-sensitivity public-data fine-tuning.
> **Nuances & gotchas:** Kubernetes network policies prevent pod-to-pod traffic but do not prevent a job from calling external services (e.g., sending data to an attacker-controlled endpoint) unless egress is also locked down to specific allowlisted destinations. The node selector for tenant isolation must be enforced consistently; a misconfigured deployment could bypass it and co-schedule with another tenant's job on the same physical node. network policies prevent egress to anything other than that tenant's S3 prefix and the central metric service; node selectors prevent co-scheduling. We also use a dedicated KMS key per tenant for both bucket encryption and model artifact encryption. A leaked artifact decryption key would expose one tenant, not all.

### 5. Serving-time isolation: shared GPU is okay, KV cache is not

The base model is shared. The adapter is per-tenant. The KV cache is per-request.

> **Why (the rationale):** Base model weights are not tenant-specific — sharing the GPU for the base model computation is safe. The KV cache, however, stores intermediate activations of each request's input tokens; if it were shared or reused across tenants, one tenant's confidential contract text could bleed into another's attention context. PagedAttention allocates KV blocks per-request and frees them on completion, providing the necessary isolation.
> **When to use:** Shared-GPU + per-request KV cache isolation is the right model for multi-tenant inference on any shared base model. Only move to dedicated GPUs per tenant if a tenant has such extreme throughput requirements that they saturate a full GPU, or if a compliance regime explicitly prohibits shared hardware.
> **Nuances & gotchas:** The red-team audit of 50K adversarial pairs is a point-in-time assurance; vLLM version upgrades can change PagedAttention internals and must be re-audited before deployment. Also, side-channel attacks (e.g., timing attacks on KV cache hit/miss patterns) are a theoretical risk in very high-security contexts that standard functional audits do not cover. PagedAttention ([vLLM paper](https://arxiv.org/abs/2309.06180)) ensures KV blocks are isolated per request, so even though Tenant A and Tenant B share a GPU during a single inference batch, their attention computations and KV state do not mix. We audited this with red-team prompts: no cross-tenant leakage in 50K adversarial pairs.
