...
> **Nuances & gotchas:** Fine-tuning does NOT give the model access to real-time data — knowledge is frozen at training time. RAG and fine-tuning can be combined (fine-tune for behavior, RAG for freshness), which is often the optimal production pattern. Continued pre-training requires **unlabeled** data; fine-tuning requires **labeled** prompt-completion pairs — confusing them causes job failures in the Bedrock API. Historically, fine-tuned Bedrock models required Provisioned Throughput to serve; as of 2025, on-demand deployment is available for select families (Amazon Nova, Meta Llama) — verify in the Bedrock docs for the specific model you use.
