...
> **Why (the rationale):** Standard chunking severs the surrounding information that makes a chunk retrievable — a chunk saying "It costs $200" contains no searchable signal. Contextual Retrieval fixes this at ingest time rather than at query time, so the cost is paid once.
> **When to use:** When your corpus has multi-section documents (reports, manuals, contracts) where chunks regularly lose meaning in isolation, your retrieval failure rate exceeds ~3–5%, or domain-specific exact terms (product IDs, section numbers) are being missed.
> **Nuances & gotchas:** This is an ingestion-time technique — it re-runs every time you re-chunk. Each chunk requires an LLM call; without prompt caching the cost is proportional to `num_chunks × document_length`. It does NOT help when chunks are already self-contained (FAQ pairs, product cards) or when the corpus is small enough to fit in a long context window.
...
> **Why (the rationale):** Embedding a raw chunk collapses its meaning into a vector that may be semantically distant from any realistic user query. Prepending a generated context string pulls the vector toward the queries that would expect that chunk, reducing the embedding space gap between the chunk and the query.
> **When to use:** Use as the default starting point for any corpus where chunks lose document context. It is the lowest-friction step — the only change is what text goes into the embedding call.
> **Nuances & gotchas:** The context string is generated by an LLM but embedded as literal text, so poor context quality directly degrades retrieval. Context generation can hallucinate if the document is very long and the chunk is marginal — spot-check a random sample. This alone gives ~35% fewer retrieval failures; combine with Contextual BM25 for 49%.
...
> **Why (the rationale):** Dense embeddings blur exact-match signal — a product ID like "Widget-X-2025" gets averaged with surrounding semantics and can score low against a keyword search for that exact ID. BM25 preserves exact-term matching, but raw chunks may lack the very terms users search for. Contextualizing the BM25 index puts those terms back in the document.
> **When to use:** Add Contextual BM25 whenever your corpus includes proprietary names, product codes, version numbers, or rare domain acronyms — i.e., anything where exact string matching matters and the terms appear in the document title/section but not in the chunk body.
> **Nuances & gotchas:** Contextual BM25 does NOT improve semantic recall on its own — it is only useful in combination with vector search (hybrid). Running it in isolation on context-enriched chunks gives no advantage over standard BM25 on full documents.
...
> **Why (the rationale):** Without caching, contextualizing a 10,000-token document split into 30 chunks sends the full document 30 times — multiplying input-token cost by chunk count. Prompt caching lets the provider reuse the processed representation of the document prefix, so only the first chunk pays the full cost.
> **When to use:** Whenever contextualizing a multi-chunk document (i.e., almost always). The break-even is roughly 2+ chunks per document; beyond that, caching is strictly cheaper.
> **Nuances & gotchas:** Prompt caching has minimum token thresholds (provider-specific); very short documents may not qualify. Cache entries expire (typically after a few minutes of inactivity), so parallelizing chunk processing too aggressively across many documents can cause cache misses. Batch documents serially, then parallelize documents.
...
> **Why (the rationale):** LLM contextualization adds latency and cost proportional to corpus size. For well-structured documents — Markdown files, HTML with heading hierarchies, PDFs with clear section titles — the document structure already encodes the context information an LLM would generate. CCH extracts it deterministically at zero marginal cost.
> **When to use:** Use CCH when documents have reliable heading hierarchies (Markdown, HTML, structured PDFs), or when real-time ingestion speed matters and per-chunk LLM calls are too slow.
> **Nuances & gotchas:** CCH is brittle on unstructured documents (legal prose, scanned PDFs, email threads) where section boundaries are absent or implicit. It does NOT generate semantic summaries — it only prepends structural metadata. If headers are absent or misleading, CCH gives no benefit over raw chunking.
