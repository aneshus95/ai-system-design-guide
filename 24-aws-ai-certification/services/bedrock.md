...
- **Single unified API for many models.** One consistent API surface — including the `Converse` API for a standardized message format and the `InvokeModel` API — lets you call and swap models from multiple providers without rewriting integration code. The `Converse` API requires the same IAM action as direct inference (`bedrock:InvokeModel`); there is no separate `bedrock:Converse` IAM action. **Why:** This means your IAM policies only need to grant `bedrock:InvokeModel` — switching from `InvokeModel` to `Converse` requires no IAM changes, making the migration frictionless.
...
- **Amazon Bedrock Agents** — break a user goal into steps, call your APIs/Lambda functions via **action groups**, query Knowledge Bases, and reason across multiple turns to complete tasks (tool use / function calling, managed for you). **Why:** Agents exist because most real-world tasks can't be answered with one LLM call — they require planning ("what steps do I need?"), tool use ("look up the order status"), and looping ("did that step succeed? what next?"). Agents handle that orchestration loop so you don't build a custom ReAct/chain-of-thought harness yourself.
- **Amazon Bedrock AgentCore** — a separate, lower-level agentic *platform* (GA October 2025) for deploying and operating agents built with *any* framework (LangGraph, CrewAI, custom). Provides managed infrastructure including: **AgentCore Runtime** (secure serverless hosting with session isolation, up to 8-hour execution windows), **AgentCore Memory** (short- and long-term memory across sessions), **AgentCore Gateway** (unified MCP-compatible tool server so agents can discover and invoke tools), **AgentCore Browser Tool** (cloud-hosted browser for web-based tasks), and **Code Interpreter**. **Why:** Bedrock Agents is an opinionated, managed orchestration layer; AgentCore is the underlying platform for teams that want to bring their own agent framework but still need enterprise-grade runtime, memory, and identity infrastructure.
- **Amazon Bedrock Guardrails** — configurable safety and responsible-AI controls applied independently of the model. **Why:** Because guardrails live outside the model, the same policy (e.g., block PII, deny investment-advice topics) can be applied consistently across every model you use, without retraining or prompt-engineering each one separately.
...
- **Serverless & fully managed** — no infrastructure to provision, automatic scaling, pay for what you use. **Why:** Foundation models require specialized GPU hardware that is expensive to idle. The serverless model means AWS pools GPU capacity across many customers, so you only pay for the compute seconds your requests actually consume — you don't pay for idle capacity between calls.
...
Knowledge Bases can create/manage vectors for you, or connect to an existing store. **Why a vector store is required:** RAG retrieval works by converting documents and queries into high-dimensional number arrays (embeddings) and finding the nearest neighbors. A traditional SQL or key-value store cannot do this efficiently — a purpose-built vector store supports the approximate nearest-neighbor (ANN) index structure that makes semantic search fast. Supported options include: **Amazon OpenSearch Serverless** (default managed option), **Amazon OpenSearch Managed Cluster** (also supports binary vectors), **Amazon Aurora PostgreSQL (pgvector)**, **Amazon Neptune Analytics** (GraphRAG for relationship-rich knowledge), **Amazon S3 Vectors** (low-cost, for infrequent-query workloads; does not support binary vectors or hybrid search), **Pinecone**, **MongoDB Atlas**, and **Redis Enterprise Cloud**.
...
- **Prompt caching** — cache repeated prompt prefixes (e.g., long system prompts/context) to cut cost and latency on repeated inputs. Cache entries have a **TTL that resets on each cache hit**: the default is **5 minutes** for most models; select Claude models (Claude Sonnet 4.5, Haiku 4.5, Opus 4.5) additionally support a **1-hour TTL** for longer-running agentic sessions. Prompt caching is **only supported for on-demand inference** — it does not work with batch inference. **Why:** Cached tokens are not re-computed, so AWS charges a reduced read rate for them; the tradeoff is a small additional write charge when the cache is first populated, and cache misses incur no discount.
- **Provisioned Throughput** — reserve dedicated model capacity in **model units**, billed **hourly**. Choose from **no commitment** (cancel any time), **1-month**, or **6-month** commitment terms (longer = more discounted). Provides guaranteed throughput for steady, high-volume traffic. **Required to run custom (fine-tuned or imported) models**, since they can't live in the shared on-demand pool. **Why Provisioned Throughput exists:** On-demand requests go into a shared pool that AWS throttles when demand spikes. If your application needs consistent, predictable latency and throughput (e.g., a high-volume production API), on-demand throttling is unacceptable. Provisioned Throughput reserves dedicated capacity so your requests always have headroom — you trade variable cost for predictable performance, similar to a Reserved Instance vs. On-Demand EC2.
...
- **"Block harmful content / deny certain topics / redact PII / reduce hallucinations in RAG"** → **Bedrock Guardrails**. There are seven policy types: (1) **Content filters** (Hate, Insults, Sexual, Violence, Misconduct — configurable per category; also covers **Prompt Attack** / jailbreaks as a Standard-tier sub-category within content filters, *not* a separate top-level policy); (2) **Denied topics** (define up to 30 off-limits subjects); (3) **Word filters** (exact-match block list, including a ready-to-use profanity list); (4) **Sensitive information filters** (PII detection and redaction via ML + custom regex); (5) **Contextual grounding checks** (hallucination detection for RAG — checks answer is grounded in retrieved passages and relevant to the query); (6) **Automated Reasoning checks** (policy-based logical validation of model outputs); and (7) **Prompt attacks** (Standard tier, detects jailbreaks/injections/leakages — offered as a content-filter sub-category). Guardrails work **across models** and can be reused.
...
- Bedrock is **regional** and supports **IAM**, **KMS encryption**, **CloudWatch/CloudTrail**, and **cross-region inference** for higher availability/throughput. **Why cross-region inference:** On-demand capacity is finite per region; a sudden traffic spike can exhaust the pool and cause throttling. Cross-region inference profiles automatically route to the same model in a geographically nearby region when your home region is congested, providing higher effective throughput without you managing multi-region logic. Note: prompt caching can be used with cross-region inference, though high-demand periods may lead to increased cache writes as requests shift regions.
...
| Provisioned Throughput | A Bedrock pricing option where you reserve dedicated model capacity billed per hour; available with no commitment, 1-month, or 6-month terms | Guarantees throughput for steady high-volume workloads and is required for custom models |
